...
class PostureCamera:
    ...
    def stop_camera(self):
        try:
            ...
            if self.sleep_start_time:
                self.sleep_duration = (self.sleep_end_time - self.sleep_start_time).total_seconds()
    ...
    def get_sleep_duration(self):
        """获取睡眠时长（秒）"""
        if self.sleep_start_time and not self.sleep_end_time:
            return (datetime.now() - self.sleep_start_time).total_seconds()
        return self.sleep_duration

    def get_start_time(self):
        """获取开始时间"""
        return self.sleep_start_time.isoformat() if self.sleep_start_time else None

    def get_end_time(self):
        """获取结束时间"""
        return self.sleep_end_time.isoformat() if self.sleep_end_time else None
